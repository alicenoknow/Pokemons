...
export default function Attack(props: {
  ...
  const sendDispatch = () => {
    ...
    dispatch(
      fightSlice.actions.decreaseHealth({
        index: attackedIndex,
        value: props.damage,
      })
    );
    if (fight.pokemons[attackedIndex].health - props.damage <= 0) {
      navigation.navigate("GameOverScreen", {
        winner: fight.pokemons[(attackedIndex + 1) % 2].name,
        loser: fight.pokemons[attackedIndex].name,
      });
    }
  };
  return (
    <TouchableOpacity
      disabled={fight.attack.isAttack}
      style={{
        ...styles.button,
        opacity: opacity,
        backgroundColor: TypeColor[props.types[0]].color,
      }}
